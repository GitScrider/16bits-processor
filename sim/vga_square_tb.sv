...
// -----------------------------------------------------------------------------
//  Drives vga_square with a fixed vid_val (as if the CPU had stored that value)
//  and captures one full visible frame (640x480) as a PPM file, so we can look at
//  exactly what the monitor would show. Also checks the frame really is 640x480
//  visible pixels. The box for vid_val=200 lands at (200,200).
//
...
module vga_square_tb;
    ...
    logic [15:0] vid_val = 16'd200;      // -> box at (200,200)
    logic        hsync, vsync, display_on;
    logic [7:0]  r, g, b;

    vga_square dut (
        .clk(clk), .rst(rst), .pix_en(pix_en), .vid_val(vid_val),
